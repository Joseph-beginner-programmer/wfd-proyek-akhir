    ...
    <main class="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="mt-2 mb-6 flex flex-col md:flex-row items-center gap-5">
            ...
            <div class="col-md-3 border-2 rounded-lg p-3  ">
                <select class="form-select search-input" aria-label="Pilih Tipe Venue" id="searchType">

                    class="w-full md:w-1/4 p-3 border-2 border-black rounded-lg font-semibold text-gray-500 focus:ring-2 focus:ring-blue-500 focus:outline-none">
        ...
        <div id="venueResults" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @include('pages.venue-cards', ['venues' => $venues])
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function fetchVenues() {
                ...
                fetch(`/venues/search?type=${type}`)
                    .then(response => response.text())
                    .then(html => {
                        document.getElementById('venueResults').innerHTML = html;
                    });
            }

            typeInput.addEventListener('change', fetchVenues);
        });

        function fetchVenues() {
            document.getElementById('venueResults').innerHTML = '<p>Loading...</p>';
