        ...
        {{ $selectedDate === $date->format('Y-m-d') ? 'bg-blue-700 text-white' : 'text-gray-800 hover:bg-blue-500 hover:text-white' }}"
                        ...
                        <div id="jadwal-grid" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                            ...
                            @foreach ($allJadwals as $jadwal)
                                @if (!$jadwal->is_active)
                                    <div class="text-center border bg-gray-50 text-gray-400 rounded-lg cursor-not-allowed">
                        ...
                        <button id="book-button" type="submit" class="mt-4 bg-blue-500 text-white px-4 py-2 rounded">Book
                            Now</button>
                    </form>
                </div>
            </div>
            ...
            <div class="lg:col-span-1 mt-10 lg:mt-0">
                <div class="lg:sticky lg:top-8 space-y-6 py-4">
                    ...
                    <div class="border border-gray-200 rounded-xl shadow-lg p-6 text-center">
                        ...
                        <p class="text-3xl font-bold text-blue-700 my-2">{{ $venue->price_per_hour }}</p>
                        <a href="#pilih"
            ...
            function updateBookingSummary() {
                ...
                if (selectedCheckboxes.length > 0) {
                    selectedCheckboxes.forEach(checkbox => {
                        ...
                        listItem.innerHTML =
                            `<span><i class="far fa-clock mr-2 text-gray-400"></i>Pkl ${time}</span> <span class="font-semibold">Rp${parseFloat(checkbox.dataset.price).toLocaleString('id-ID')}</span>`;
