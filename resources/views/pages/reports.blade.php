        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function renderUsersReport(data) {
                ...
                <tr class="${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}">
                    ...
                    <td class="td-cell font-medium text-gray-900 text-center">${user.name}</td>
                    <td class="td-cell text-center">${user.email}</td>
                    <td class="td-cell">
            ...
            function renderBookingsReport(data) {
                ...
                data.forEach((booking, index) => {
                    const row = `
        <tr class="${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}">
            <td class="td-cell text-gray-900 font-mono text-center">${booking.booking_id}</td>
            <td class="td-cell text-center">${booking.user?.name ?? '-'}</td>
            <td class="td-cell text-center">${booking.venue?.name ?? '-'}</td>
            <td class="td-cell text-center">${getStartTime(booking.booking_hours)}</td>
            <td class="td-cell text-center">${getEndTime(booking.booking_hours)}</td>
            <td class="td-cell text-center"><span class="${getBookingStatusClass(booking.booking_status)}">${booking.booking_status}</span></td>
            <td class="td-cell text-center">${new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(booking.payment?.total_price ?? 0)}</td>
            <td class="td-cell text-center">${new Date(booking.created_at).toLocaleString('id-ID')}</td>
        </tr>
    `;
                    tableBody.insertAdjacentHTML('beforeend', row);
                });
            ...
            function renderFinancialReport(data) {
                ...
                data.transactions.forEach((trx, index) => {
                    const row = `
                <tr class="${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}">
                    <td class="td-cell font-mono text-center">${trx.payment_id}</td>
                    <td class="td-cell font-mono text-center">${trx.booking_id}</td>
                    <td class="td-cell text-center">${trx.customer_name}</td>
                    <td class="td-cell text-center">${new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(trx.total_price)}</td>
                    <td class="td-cell text-center">${trx.payment_method}</td>
                    <td class="td-cell text-center">${new Date(trx.paid_at).toLocaleString('id-ID')}</td>
                </tr>
            `;
                    tableBody.insertAdjacentHTML('beforeend', row);
                });
